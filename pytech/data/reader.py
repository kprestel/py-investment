...
from pytech.data.schema import assets, bars
from pytech.decorators import write_df
from pytech.exceptions import DataAccessError
from pytech.sources import AlphaVantageClient, TiingoClient
...
class BarReader(object):
    ...
    def get_data(
        ...
        **kwargs,
        ...
        if isinstance(tickers, str):
            try:
                result = self._single_get_data(
                    tickers, source, date_range, check_db, **kwargs
                )
                return result.df
            except DataAccessError:
                raise RestClientError(f"Could not get data for ticker: " f"{tickers}")
        else:
            if isinstance(tickers, pd.DataFrame):
                tickers = tickers.index
            try:
                return self._mult_tickers_get_data(
                    tickers, source, date_range, check_db, **kwargs
                )
            except DataAccessError:
                raise

    def _mult_tickers_get_data(
        self,
        tickers: List,
        source: str,
        ...
        **kwargs,
        ...
        if len(stocks) > 0 and len(failed) > 0 and len(passed) > 0:
            ...
            for t in failed:
                logger.warning(
                    f"No data could be retrieved for ticker: {t}, "
    ...
    def _single_get_data(
        self, ticker: str, source: str, date_range: DateRange, check_db: bool, **kwargs
    ) -> ReaderResult:
        ...
        if check_db:
            try:
                ...
                logger.info(
                    f"Ticker: {ticker} not found in DB, attempting to"
        ...
        try:
            ...
            logger.warning(
                f"Error getting data from {source} " f"for ticker: {ticker}", exc_info=1
            )
            return ReaderResult(ticker, successful=False)

    @write_df("bar")
    def _from_web(
        self, ticker: str, source: str, date_range: DateRange, **kwargs
    ) -> ReaderResult:
        ...
        try:
            ...
            if df.empty:
                raise RestClientError(
                    f"df retrieved was empty for " f"ticker: {ticker}."
                )
        except RestClientError as e:
            logger.warning(
                f"Unable to retrieve intra day data for "
    ...
    def _from_db(
        ...
        """
        q = sa.select([bars]).where(
            sa.and_(
                bars.c.date >= date_range.start,
...
def _concat_dfs(
    lower_df: pd.DataFrame, upper_df: pd.DataFrame, df: pd.DataFrame
) -> pd.DataFrame:
    """
