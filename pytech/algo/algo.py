...
class Algorithim(metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        self.start_date = kwargs.pop(
            "start_date", dt_utils.get_default_date(is_start_date=True)
        )
        self.end_date = kwargs.pop(
            "end_date", dt_utils.get_default_date(is_start_date=False)
        )
        ...
        if self.blotter is None:
            self.blotter = Blotter(asset_finder=self.asset_finder)
