    ...
    def generate_signals(self, event: MarketEvent):
        ...
        if event.event_type is EventType.MARKET:
            ...
            for ticker in self.ticker_list:
                ...
                bars = self.bars.latest_bar_value(ticker, pd_utils.ADJ_CLOSE_COL)
...
class CrossOverStrategy(Strategy):
    ...
    def generate_signals(self, event: MarketEvent):
        ...
        for ticker in self.ticker_list:
            ...
            signals["short_mavg"] = (
                signals[pd_utils.CLOSE_COL]
                .rolling(
                    center=False,
                    window=self.short_window,
                    min_periods=self.short_window - 1,
                )
                .mean()
            )
            signals["long_mavg"] = (
                ...
                .rolling(
                    ...
                    min_periods=self.long_window - 1,
                )
                .mean()
            )
            ...
            short = short.iat[0]
            long = long.iat[0]
            self.logger.debug(f"Ticker: {ticker}, long: {long}, short: {short}")
