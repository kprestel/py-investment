...
def get_symbol(values):
    if values:
        symbols = [s.strip() for s in values[0].split(',')]
        return '/'.join(symbols)
    return False
...
def imd_get_revenues(imd_values):
    interest_elems = [v for v in imd_values if 'interest' in v.local_name.lower()]
...
def imd_get_equity(imd_values):
    ...
    values = [v for v in imd_values if v.local_name == 'StockholdersEquityIncludingPortionAttributableToNoncontrollingInterest']
    if values:
        return values[0].value

    values = [v for v in imd_values if v.local_name == 'StockholdersEquity']
    if values:
        return values[0].value
